...
public class SlidingEllipsesRayAlgorithm implements IShadowAlgorithm{
	...
	public void castShadow(Canvas canvas, AbstractCaster<? extends AbstractSection> caster, GC gc) {
		...
		int leftCharacterBorder = clientArea.width/2 - caster.getSheetWidth()*pixelSize/2;
		int bottomCharacterBorder = clientArea.height/2 + caster.getSheetHeight()*pixelSize/2;
		Bounds[] frontBounds = caster.getLeftToRightBounds();
		Bounds[] rightBounds = caster.getFrontToBackBounds();
		int bottomOffsetY = rightBounds[rightBounds.length-1].getLowerBound();
		
		for(int row = caster.getSheetHeight()-1; row >=0;row--) {
			int axisX1 = frontBounds[row].getWidth()*pixelSize;
			int axisY1 = (int) (rightBounds[row].getWidth()*pixelSize*inclinationFactor);
			int distanceFromBottom1 = caster.getSheetHeight()-row-1;
			...
			if(row == 0) {
				...
				int distanceFromBottom2 = caster.getSheetHeight()-row;
