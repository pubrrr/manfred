...
public class SlidingEllipsesRayAlgorithm implements IRayAlgorithm{
	...
	public void castShadow(Canvas canvas, Character character, GC gc) {
		...
		for(int row = character.getSheetHeight()-1; row >=0;row--) {
			int axisX1 = (frontBounds[row][1] - frontBounds[row][0] + 1)*pixelSize;
			int axisY1 = (int) ((rightBounds[row][1] - rightBounds[row][0] + 1)*pixelSize*inclinationFactor);
			int distanceFromBottom1 = character.getSheetHeight()-row-1;
			int centerX1 = leftCharacterBorder + pixelSize*frontBounds[row][0] + (int) Math.round(distanceFromBottom1*relativeProjectionShiftX*pixelSize) + axisX1/2;
			int centerY1 = bottomCharacterBorder + (bottomOffsetY - rightBounds[row][0])*pixelSize - axisY1/2 + (int) Math.round(distanceFromBottom1*relativeProjectionShiftY*pixelSize);
			if(row == 0) {
				gc.fillOval(centerX1 - axisX1/2, centerY1 - axisY1/2, axisX1, axisY1);
			} else {
				int axisX2 = (frontBounds[row-1][1] - frontBounds[row-1][0] + 1)*pixelSize;
				int axisY2 = (int) Math.round((rightBounds[row-1][1] - rightBounds[row-1][0] + 1)*pixelSize*inclinationFactor);

				int distanceFromBottom2 = character.getSheetHeight()-row;
				int centerX2 = leftCharacterBorder + pixelSize*frontBounds[row-1][0] + (int) Math.round(distanceFromBottom2*relativeProjectionShiftX*pixelSize) + axisX2/2;
				int centerY2 = bottomCharacterBorder + (bottomOffsetY - rightBounds[row-1][0])*pixelSize - axisY2/2 + (int) Math.round(distanceFromBottom2*relativeProjectionShiftY*pixelSize);
